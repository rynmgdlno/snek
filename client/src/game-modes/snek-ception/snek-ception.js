...
import './snek-ception.scss'

const SnekCeption = () => {
  const gridSize = 20
  const fillColor = 'rgba(230,24,19,1.0)'
  ...
  return (
    <div className='page-container-ception'>
      <Header fillColor={fillColor} />
      <div className='border-ception'></div>
      <h4 className='mode-title-ception'>S N E K C E P T I O N</h4>
      <div className='game-container-ception'>
        <div className='game-info-ception'>
          <div className='mode-bar-ception'>
            <p className='speed-text-ception' style={{ color: fillColor }}>{speedLevel}</p>
            <div className='controls-ception'>
              <Difficulty
          ...
          <div className='stats-ception'>
            <h5 className='stats-text-ception'>{length}</h5>
            <h5 className='stats-text-ception'>Multiplier: {totalMult}</h5>
            <h5 className='score-ception'>SCORE: <span className='points-ception'>{points}</span></h5>
          </div>
          <HighScores gameMode='SNEKCEPTION' fillColor={fillColor}/>
        </div>
        <div className='board-container-ception'>
          <div className='button-container-horizontal-ception' onClick={() => { handleArrowPress('up') }}>
            <UpButton fillColor={fillColor} />
          </div>
          <div className='secondary-container-ception'>
            <div className='button-container-vertical-ception' onClick={() => { handleArrowPress('left') }}>
              <LeftButton fillColor={fillColor} />
            </div>
            <div className='board-ception'>
              {board.map((space) => {
                const id = `${space.x}${'-'}${space.y}`;
                return <div
                  className={snakeIDs.includes(id) ? 'snake-ception space-ception' :
                    foodSpace === id ? 'food-ception space-ception' :
                      superFoodSpace === id ? 'super-food-ception space-ception' :
                        megaFoodSpace === id ? 'mega-food-ception space-ception' : 'space-ception'}
                  key={id}
                  id={id}>
                </div>
              })}
            </div>
            <div className='button-container-vertical-ception' onClick={() => { handleArrowPress('right') }}>
              <RightButton fillColor={fillColor} />
            </div>
          </div>
          <div className='button-container-horizontal-ception' onClick={() => { handleArrowPress('down') }}>
            <DownButton fillColor={fillColor} />
          </div>
        </div>
      </div>
      {
        endGame && <EndGame snakeLength={snake.length} fillColor={fillColor} score={points} gameMode='SNEKCEPTION' difficulty={speedLevel}/>
      }
    </div>
  )
}

export default SnekCeption